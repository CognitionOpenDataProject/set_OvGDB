...
NB - author confirmed this was the case - the article should have said greater than or equal to midpoint, as below:

```{r}
dat %>% select(Low.Blame) %>% filter(Low.Blame>=0) %>% nrow() / dat %>% select(Low.Blame) %>% nrow()
compareValues(reportedValue = .31, obtainedValue = 0.3125)
```
...
# p.value
compareValues(reportedValue = .24, obtainedValue = t.low$p.value)
```
